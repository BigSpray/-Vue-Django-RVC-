<template>
    <div style="display: flex;">
      <div style="flex: 1;">
        <div style="justify-content: space-between;margin: 20px 5px;padding:0 0 0 15px;border-left: 10px solid #15c804;display: flex;align-items: center;">
        ...
        <div class="card" style="height: 707.55px; position: relative;">
          <div v-if="displayblog">
              ...
              <div @click="gohorseshow('/main/detail?id=' + item.id, item)" style="cursor: pointer; padding: 15px;display: flex;justify-content: space-between;flex: 1;border-bottom: 1px dotted #15c804;">
                <div>
                ...
                <div v-if="item.img" style="display: flex;align-items: center;">
                  <el-image
                    ...
                    style="width: 120px; object-fit: cover;border-radius: 10px;"
            ...
            <div style="position: absolute; bottom: 0; width: 100%; padding: 10px; box-sizing: border-box;">
              <el-pagination :current-page="pagenum1" :page-size="4" style="margin-top: 10px;" background layout="total, prev, pager, next" :total="total" @update:current-page="onCurrentPageChange1"
              />
            </div>
          </div>
          <div v-else>
            <div style="height: 570px;" v-loading="load" element-loading-text="加载中" element-loading-background="rgb(10,154,243 ,0.1)">
            </div>  
          </div>
        </div>
      </div>
    ...
    methods: {
      ...
      getData() {
        ...
        axios.get("http://127.0.0.1:8000/getblogs").then((res) => {
          ...
          console.log('获取到的所有博客数据:', this.all); // 打印所有博客数据
          if (this.searchtitle) {
            this.filteredBlogs = this.all.filter(item => item.title.includes(this.searchtitle));
          } else {
            console.log(this.searchname);
            this.filteredBlogs = this.all.filter(blog => blog.name === this.searchname);
          }
          // 按照最后编辑时间排序，取最新的六个博客
          const sortedByTime = this.filteredBlogs.sort((a, b) => {
