<template>
    ...
    <el-dialog v-model="dialogVisible2" title="添加公告/活动">
...
export default {
    ...
    methods: {
        ...
        getData() {
            axios.get("http://127.0.0.1:8000/getblogs")
              .then((res) => {
                    ...
                    const filteredBlogs = all.filter(blog => blog.forblog === '公告' || blog.forblog === '活动');
                    this.allData = filteredBlogs;
        ...
        addblog(){
            ...
            formData.append('type', this.newblog.forblog);
            formData.append('content', this.newblog.content);
            formData.append('forblog', this.newblog.forblog);
            const response = axios.post('http://127.0.0.1:8000/saveBlog/', formData, {
                    headers: {
